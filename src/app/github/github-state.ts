...
function buildInitialSnapshot(): GitHubSnapshot {
  const repos: Record<string, GitHubRepo> = {
    "corp-os": {
      ...
      owner: "corp",
      name: "corp-os",
      description: "Internal operating system app replicas and workflow surfaces.",
      language: "TypeScript",
    ...
    "data-sync": {
      ...
      owner: "corp",
      name: "data-sync",
      description: "Mock sync layer for future local/cloud persistence work.",
      language: "Go",
      memberIds: ["riley", "ari"],
      starredBy: ["ari"],
      watchedBy: ["riley", "ari"],
  ...
  const issueTitles = [
    ...
    "Mock data needs cross-user fixtures",
  ...
  Object.values(repos).forEach((repo, repoIndex) => {
    for (let index = 0; index < 8; index += 1) {
      ...
      const issueComments = [
        {
          ...
          body: "I can reproduce this on the current mock route.",
      ...
      issues[id] = {
        ...
        body: "This issue tracks a product-level workflow gap found during replica QA.",
  ...
  Object.values(repos).forEach((repo, repoIndex) => {
    for (let index = 0; index < 6; index += 1) {
      ...
      pulls[id] = {
        ...
        body: "This pull request updates the replica surface and includes mock workflow coverage.",
        status: index === 4 ? "merged" : index === 5 ? "closed" : "open",
        sourceBranch: `feature/replica-${index + 1}`,
        ...
        changedFiles: [
          { path: "src/app/replica/App.tsx", additions: 180 + index * 12, deletions: 20 + index },
          { path: "src/app/replica/state.ts", additions: 90 + index * 8, deletions: 4 },
        ],
  ...
  Object.values(repos).forEach((repo) => {
    [
      ...
      [
        "src/app.tsx",
        "file",
        "export function App() {\n  return <main>Replica surface</main>;\n}\n",
      ],
