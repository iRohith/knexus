...
function normalizeTab(value: string | null): GitHubTab {
  return githubTabs.includes(value as GitHubTab) ? (value as GitHubTab) : "code";
}
...
export function GitHubApp() {
  ...
  useEffect(() => {
    ...
    router.replace(`${pathname}?repo=${nextRepo}&tab=code`);
              ...
              <TabsList className="bg-transparent">
                ...
                <TabsTrigger className="cursor-pointer gap-2" value="issues">
                  ...
                  <Badge variant="secondary" className="ml-1 h-5 rounded-full px-1.5">
                    {repoIssues.filter((issue) => issue.status === "open").length}
                  </Badge>
                </TabsTrigger>
                <TabsTrigger className="cursor-pointer gap-2" value="pulls">
                  ...
                  Pull requests
                  <Badge variant="secondary" className="ml-1 h-5 rounded-full px-1.5">
                    {repoPulls.filter((pull) => pull.status === "open").length}
                  </Badge>
                </TabsTrigger>
...
function PullList({ pulls, onOpen }: { pulls: GitHubPullRequest[]; onOpen: (id: string) => void }) {
  if (pulls.length === 0) {
    return (
  ...
  return (
    ...
    >
      <div className="mb-4 flex flex-wrap gap-2">
        <Badge variant={issue.status === "open" ? "secondary" : "outline"}>{issue.status}</Badge>
        {issue.labels.map((label) => (
...
function CodeView({
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-[20rem_1fr]">
      ...
      <div className="overflow-hidden rounded-md border border-[#d0d7de] bg-white dark:border-[#30363d] dark:bg-[#0d1117]">
        <div className="border-b border-[#d0d7de] bg-[#f6f8fa] px-3 py-2 font-mono text-xs dark:border-[#30363d] dark:bg-[#161b22]">
        ...
        {selectedContent ? (
          ...
          <div className="p-8 text-center text-sm text-muted-foreground">
            Select a matching file to preview its contents.
          </div>
        )}
      </div>
    </div>
  ...
  return (
    <div className="overflow-hidden rounded-md border border-[#d0d7de] bg-white dark:border-[#30363d] dark:bg-[#0d1117]">
      {notifications.map((notification) => (
        <button
          key={notification.id}
          className="flex w-full cursor-pointer items-center gap-3 border-b border-[#d0d7de] p-4 text-left last:border-b-0 hover:bg-[#f6f8fa] dark:border-[#30363d] dark:hover:bg-[#161b22]"
