...
test.describe("Corp-OS Data Density E2E Tests", () => {
  ...
  test.describe("Gmail", () => {
    ...
    test("can page through emails (pagination works)", async ({ page }) => {
      ...
      const nextBtn = page
        .locator('button[aria-label*="next"], button:has-text("Next"), button[aria-label*="Next"]')
        .first();
  ...
  test.describe("Slack", () => {
    ...
    test("can scroll up to load more messages", async ({ page }) => {
      ...
      const messagesArea = page
        .locator('main, [class*="messages"], [class*="scroll"], .overflow-y-auto')
        .last();
  ...
  test.describe("User Menu", () => {
    test("shows all 10 demo users in switcher", async ({ page }) => {
      ...
      const userMenuTrigger = page
        .locator('button[aria-label*="Current user"], button[aria-label*="user"]')
        .first();
