...
import { ComposeBox } from "@/app/slack/ComposeBox";
...
export function SlackApp({
  ...
  useEffect(() => {
    ...
    async function loadInitialCorpus() {
      ...
      try {
        const app = { pageCount: 1, count: 100 };
        if (cancelled) return;
  ...
  const surfaceMessages = useMemo(
    () =>
      Object.values(messages)
        ...
        .slice(-100), // RENDER ONLY THE LATEST 100 MESSAGES
  ...
  const threadReplies = useMemo(
    () =>
      Object.values(messages)
        ...
        .sort((a, b) => a.timestamp - b.timestamp)
        .slice(-100), // RENDER ONLY LATEST 100 REPLIES
    [messages, threadId],
  );
  ...
  function sendCurrentMessage(draft: string, attachments: SlackAttachment[]) {
  ...
  function sendThreadReply(threadDraft: string, threadAttachments: SlackAttachment[]) {
    ...
    <main className="flex h-[calc(100vh-3.5rem)] min-h-0 bg-[#f8f5f8] text-sm text-foreground dark:bg-[#1d1028]">
      ...
      <section className="flex min-w-0 flex-1 flex-col bg-white text-foreground dark:bg-[#1a1d21] dark:text-zinc-100">
        ...
        <ComposeBox
          ...
          makeAttachment={() => makeAttachment(`slack-${Date.now()}`)}
        />
      </section>
        ...
        canAccessSurface(
          ...
          <aside className="hidden w-104 shrink-0 border-l bg-white text-foreground xl:flex xl:flex-col dark:border-white/10 dark:bg-[#1a1d21] dark:text-zinc-100">
            ...
            <ComposeBox
              ...
              makeAttachment={() => makeAttachment("thread")}
              isThread
            />
          </aside>
        )}
