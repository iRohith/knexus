...
export function HubSpotApp({
  ...
  const accessibleCompanies = useMemo(
    () => Object.values(companies).filter(() => canAccessCompany()),
    [companies],
  );
  ...
  useEffect(() => {
    const params = new URLSearchParams(searchParams.toString());
    let changed = false;
    if (contactId && !canAccessContact()) {
      params.delete("contact");
      changed = true;
    }
    if (dealId && !canAccessDeal()) {
      params.delete("deal");
      changed = true;
    }
  ...
  const visibleTasks = Object.values(tasks)
    .filter((task) => task.ownerId === activeUser.id && canAccessContact())
    .filter(
      (task) =>
        !normalizedQuery ||
    ...
    .slice(0, 100);
  const selectedContact = contactId && canAccessContact() ? contacts[contactId] : null;
  const selectedDeal = dealId && canAccessDeal() ? deals[dealId] : null;
