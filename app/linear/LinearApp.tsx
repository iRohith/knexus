...
export function LinearApp({
  ...
  const accessibleTeams = useMemo(
    () =>
      Object.values(teams).filter((team) => activeUser?.id && canAccessTeam(team, activeUser.id)),
    [activeUser, teams],
  );
  const fallbackTeam = accessibleTeams[0]?.id ?? "core";
  const teamParam = searchParams.get("team");
  const teamId =
    activeUser?.id && canAccessTeam(teams[teamParam ?? ""], activeUser.id)
  ...
  useEffect(() => {
    ...
    const firstAccessibleTeam =
      Object.values(teams).find((item) => activeUser?.id && canAccessTeam(item, activeUser.id))
        ?.id ?? "des";
    router.replace(`${pathname}?team=${firstAccessibleTeam}&view=board`);
  }, [activeUser, pathname, router, teams]);

  useEffect(() => {
    const params = new URLSearchParams(searchParams.toString());
    let changed = false;
    if (!activeUser || !canAccessTeam(teams[teamParam ?? ""], activeUser.id)) {
          ...
          <div className="p-4">
            {selectedIssue ? (
              <IssueDetail
                ...
                onCycle={(cycle) => {
                  if (activeUser) updateCycle(selectedIssue.id, activeUser.id, cycle);
                }}
                onProject={(project) => {
                  if (activeUser) updateProject(selectedIssue.id, activeUser.id, project);
                }}
                onEstimate={(estimate) => {
                  if (activeUser) {
                    updateEstimate(
                ...
                onSubscribe={() => {
                  if (activeUser) toggleSubscriber(selectedIssue.id, activeUser.id);
                }}
                onComment={() => {
                  if (selectedIssue) {
